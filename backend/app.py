...
def prepare_interaction_matrix(interaction_matrix):
    # Create an empty DataFrame with the appropriate dimensions
    interaction_degree = pd.DataFrame('-', index=[i+1 for i in range(12)], columns=[i+1 for i in range(12)])
    
    # Populate the DataFrame with the values from the interaction_matrix
    for key, value in interaction_matrix.items():
        ...
        interaction_degree.loc[predecessor, successor] = value
