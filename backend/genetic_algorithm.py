...
class GeneticAlgorithm:
        ...
        while True:  # keep trying until a valid individual is generated
            ...
            for module in module_sequence:
                ...
                if self.module_profile[module]['type'] == 'inhouse':
                    ...
                    is_allocated = False
                    available_periods = list(range(1, int(104 - duration + 1)))
    ...
    def fitness_function(self, individual):
        ...
        products = {
            'P1': {'name': 'P1', 'ILT': 28, 1: 1, 2: 0, 3: 0, 4: 1, 5: 0, 6: 1, 7: 1, 8: 0, 9: 0, 10: 0, 11: 0, 12: 1},
            'P2': {'name': 'P2', 'ILT': 28, 1: 1, 2: 0, 3: 0, 4: 0, 5: 1, 6: 1, 7: 1, 8: 0, 9: 0, 10: 1, 11: 0, 12: 0},
            'P3': {'name': 'P3', 'ILT': 28, 1: 1, 2: 0, 3: 0, 4: 1, 5: 0, 6: 1, 7: 1, 8: 0, 9: 0, 10: 0, 11: 1, 12: 0},
            'P4': {'name': 'P4', 'ILT': 30, 1: 0, 2: 1, 3: 0, 4: 0, 5: 1, 6: 1, 7: 0, 8: 1, 9: 0, 10: 1, 11: 0, 12: 0},
            'P5': {'name': 'P5', 'ILT': 30, 1: 0, 2: 1, 3: 0, 4: 1, 5: 0, 6: 1, 7: 0, 8: 1, 9: 0, 10: 0, 11: 1, 12: 0},
            'P6': {'name': 'P6', 'ILT': 30, 1: 0, 2: 1, 3: 0, 4: 1, 5: 0, 6: 1, 7: 0, 8: 1, 9: 0, 10: 0, 11: 0, 12: 1},
            'P7': {'name': 'P7', 'ILT': 32, 1: 0, 2: 0, 3: 1, 4: 1, 5: 0, 6: 1, 7: 0, 8: 0, 9: 1, 10: 0, 11: 1, 12: 0},
            'P8': {'name': 'P8', 'ILT': 32, 1: 0, 2: 0, 3: 1, 4: 0, 5: 1, 6: 1, 7: 0, 8: 0, 9: 1, 10: 1, 11: 0, 12: 0}
        }
        ...
        total_cost = 0
        family_product_costs = {}
        for family, products_list in families.items():
            product_costs = {prod: self.calculate_product_cost(products[prod], individual) for prod in products_list}
            selected_product = min(product_costs, key=product_costs.get)
            family_cost = product_costs[selected_product]
            total_cost += family_cost
            family_product_costs[family] = {selected_product: family_cost}
        
        return total_cost, family_product_costs

    def calculate_product_cost(self, product, individual):
        product_cost = 0
        max_period = 0  # track the latest module completion time for the product
        
        # sum the costs of the modules required for the product
        for module in product:
            if product[module] == 1:  # check if the module is required for the product
                ...
                completion_time = start_period + duration - individual[module][2] if self.module_profile[module]['type'] == 'inhouse' else product['ILT']
                max_period = max(max_period, completion_time)
                
                # add base cost for all periods and crashing cost 
                if self.module_profile[module]['type'] == 'inhouse':
                    product_cost += sum(self.module_profile[module]['cost'][period] for period in range(int(start_period-1), int(completion_time)))
                    product_cost += self.module_profile[module]['crash cost'][0] * (individual[module][2] ** 2)
                else:
                    product_cost += self.module_profile[module]['cost'][individual[module][1] - 1]
                    product_cost += self.module_profile[module]['crash cost'][individual[module][1] - 1] * individual[module][2]

        # calculate delay penalty
        delay = max(0, max_period - product['ILT'])
        delay_penalty = 1000 * (delay ** 2)
        
        return product_cost + delay_penalty
        ...
        for individual in offspring:
            if random.random() < mutation_rate:
                ...
                # mutating outsourced modules
                    ...
                    # Attempt to find a valid new start period
                    available_periods = list(range(1, int(104 - lead_time + 1)))
                    random.shuffle(available_periods)
                    valid_mutation = False
